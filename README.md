...
NPM 网关模式不会让 Kopia 直接监听公网接口，但与 NPM 位于相同 Docker 网络的其他
容器也能访问该网关地址及端口。自动识别不到 NPM 时，菜单会列出运行容器，可输入
实际 NPM 容器名称或 ID 读取网关；若 NPM 使用 Docker `host` 网络，则应选择本机
`127.0.0.1` 模式。

对于 Docker 后端服务，端口监听助手的 Docker 映射菜单提供“Docker NPM 共享网络
指引”。它检查 NPM 与应用是否共享用户自定义网络；没有时生成持久化的 Compose 网络
片段，并提示使用 `应用容器名:容器内部端口` 作为 NPM 上游。该指引不自动连接网络或
重建容器。
