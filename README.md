...
宿主机监听列表中显示的 `docker-proxy` 是 Docker 对外发布的宿主机地址；容器内部
应用监听位于独立网络空间，NPM 共享网络应使用容器内部端口和 Compose 服务名访问。
端口映射菜单的选项 2、3、4 修改的都是宿主机发布地址，不会改变容器内应用的监听端口。
在“本机服务端口监听助手”的端口输入处键入 `d`，可查看全部运行容器的实际内部监听；
若容器没有 `ss` 或 `netstat`，脚本只显示镜像声明端口并明确提示不能代表实际监听。
首选“Docker NPM 共享网络创建”：它检查 NPM 与应用是否共享用户自定义网络；没有时生成
持久化的 Compose 网络片段，默认网络名按已识别的容器名称自动生成，例如
`npm-rss-reader`；仅确认 `Y` 后才会自动创建该 Docker 网络，直接回车默认不创建。创建后可
再次确认，让脚本将两个当前运行容器接入网络并检查成员关系；接入成功即可使用
`Compose 应用服务名:容器内部端口` 作为 NPM 上游。脚本不自动修改 Compose 或重建容器，
因此仍应保存输出的 Compose 片段，以免未来重建容器后连接丢失。也提供“经宿主机网关访问”的兼容方案，但它必须把
容器端口发布到 `0.0.0.0`，NPM 使用 `172.17.0.1:宿主机端口`，应以防火墙限制 Docker
网段，不如共享网络安全。菜单按“推荐、宿主机 Caddy、兼容、公网”明确标注访问路径。
...
增加端口映射。对于已识别的单文件 Compose 项目，端口映射菜单可在确认 `Y` 后自动
备份配置、精确替换当前映射、校验 Compose、重建对应服务并验证结果；失败时恢复原配置
并尝试恢复服务。多 Compose 文件、变量化或无法唯一匹配端口映射的配置仍只显示手工片段。

该映射供宿主机上的 Caddy、Nginx 等反代服务；Docker 内的 Nginx Proxy Manager
不能通过 `127.0.0.1:宿主机端口` 访问应用。但它可与共享网络方案共用：NPM 通过
`Compose 应用服务名:容器内部端口` 直连应用，宿主机反代仍使用 `127.0.0.1:宿主机端口`。
